...
特に、LLM推論システムのどこで時間やリソースが使われているのかを実測し、ボトルネックを特定した上で、**Kernel / Runtime / Serving Infrastructure / Edge Deploymentをまたいで改善すること**に関心があります。

**Kernel / CUDA → Inference Runtime → Serving Infrastructure → Edge Deployment**
...
## Selected Work
### [cuda-kernel-engineering](https://github.com/CHIPMUNK-T0T/cuda-kernel-engineering)

vLLMのdecode処理をプロファイルし、mini decoder、Attention / KV Cache、GEMV、RMSNorm、elementwise fusionなどを段階的に検証しています。
CUDA / Tritonによるカーネル実装とend-to-end検証まで行い、実際のvLLM decode workloadで**TPOTを約15%改善**するケースを確認しました。
**Result: ~15% lower TPOT in end-to-end vLLM decode**

### [consumer-gpu-llm-bench](https://github.com/CHIPMUNK-T0T/consumer-gpu-llm-bench)
...
### [edge-llm-cpu-arm64](https://github.com/CHIPMUNK-T0T/edge-llm-cpu-arm64)
...
My main areas of interest include **LLM inference with vLLM / llama.cpp / Ollama, KV Cache / Prefix Cache, speculative decoding, quantization, and reproducible performance evaluation**. I also work on **CUDA / Triton kernel optimization** and inference infrastructure spanning **Kubernetes / K3s, Helm, gateways, observability, and recovery engineering**.
I am particularly interested in measuring where LLM inference systems actually spend time and resources, identifying bottlenecks, and improving systems across **Kernel / Runtime / Serving Infrastructure / Edge Deployment**.
...
I profile vLLM decode workloads and progressively explore mini decoders, Attention / KV Cache, GEMV, RMSNorm, and elementwise fusion.
